...
export default function ComplaintRegister({ redirectTo = "/login" }) {
  ...
  const filteredComplaints = useMemo(() => {
    return complaints.filter(c => {
      const matchesStatus =
        filterStatus === "all" ||
        c.status?.toLowerCase().replace(/\s/g, "") === filterStatus.toLowerCase();
      const matchesSearch = 
  ...
  // --- Progress line helpers (Pending -> In Progress -> Resolved) ---
  ...
  function StatusProgress({ status }) {
    const step = getStatusStep(status); // 1, 2, 3
    const activeWidth = step === 1 ? "0%" : step === 2 ? "50%" : "100%";
    ...
    return (
      <div className="mt-2 text-[11px] text-gray-500">
        ...
        <div className="relative mb-3 mt-1">
          ...
          {/* Active filled line */}
          <div
            className="h-1 rounded-full bg-indigo-500 absolute inset-y-0 left-0 transition-all duration-300"
            ...
            {[1, 2, 3].map((i) => {
              ...
              return (
                <div key={i} className="flex flex-col items-center">
                  <div
                    className={
                      "w-4 h-4 rounded-full border-2 shadow-sm transition-all duration-300 " +
                      (isActive
        ...
        <div className="flex justify-between text-[10px] font-medium mb-1">
          <span className={step >= 1 ? "text-indigo-700" : "text-gray-400"}>
            Pending
          </span>
          <span className={step >= 2 ? "text-indigo-700" : "text-gray-400"}>
            In Progress
          </span>
          <span className={step >= 3 ? "text-indigo-700" : "text-gray-400"}>
            Resolved
          </span>
        </div>
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      {/* Progress line */}
                      <StatusProgress status={c.status || "Pending"} />
                    </div>
            ...
            <div className="p-6">
              <div className="flex items-start justify-between mb-6">
                <div className="flex-1">
                  ...
                  {/* Progress line inside modal as well */}
                  <StatusProgress status={selectedComplaint.status || "Pending"} />
                </div>
                <button 
